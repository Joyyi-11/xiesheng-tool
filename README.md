...
校订与结构化可以走自动化 API，也可以交给 AI 会话——后者在 API Key 无余额或不便配置时非常实用。两条链路共用同一份转写与说话人区分结果，只在"后处理"这一步分叉。两条链路的校订都要调用大模型并产生费用，区别只在计费方式：api 链路按 token 计费，session 链路消耗所在 AI 会话的积分。

- **会话内（默认）**：工具只完成转写与说话人区分，产出"会话输入包"——一份自包含文件，包含节目标题、来源、Show Notes 与带 `[SPEAKER_XX]` 标签的全文。内部处理方式已固化为固定规范（`src/processor/session_edit.py`，规范版本见该文件的 `SESSION_SPEC_VERSION`），规范提示词要求产出与自动化链路相同的完整结构——摘要、核心观点、问题与思考、术语表、人物简介与原文转录——并用内置校验器复核会话产出（含「原文转录 ≥ 原始转录 50%」的信息量硬线）。未配置 API Key 时自动走此链路。校订同样由大模型完成，按会话积分计费——和 api 链路一样要付费，只是计费方式不同（不走 API 账单）。
...
- 转写与说话人区分：本地免费（本地 ASR，不调用云端）
- 会话内链路（session）：校订由会话内 agent 完成，按会话积分计费
- 自动化链路（api）：校订按 token 计费，费用取决于模型、节目长度和所选 API 提供方账单
- 两条校订链路都要付费，区别只是计费方式（token vs 积分），没有哪条是免费的
